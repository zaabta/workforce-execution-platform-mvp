...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.78\textheight,keepaspectratio]{prism-uploads/table_style_er (1).png}
...
Authorization is implemented using Role-Based Access Control (RBAC) combined
with organizational scopes.

Each user is assigned one or more roles (e.g., Technical Office Engineer, Head
of Master, Site Chief, or Project Manager), while permissions define the
actions each role can perform.

\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[>=stealth,
        scopebox/.style={draw,rounded corners,align=center,minimum width=2.5cm,
                         minimum height=0.7cm}]
        \node[scopebox] (project) at (0,0) {Project};
        \node[scopebox] (regiona) at (-3,-1.5) {Region A};
        \node[scopebox] (regionb) at (3,-1.5) {Region B};
        \node[scopebox] (location1) at (-4.5,-3) {Location 1};
        \node[scopebox] (location2) at (-1.5,-3) {Location 2};
        \node[scopebox] (location3) at (1.5,-3) {Location 3};
        \node[scopebox] (location4) at (4.5,-3) {Location 4};

        \draw[->,thick] (project) -- (regiona);
        \draw[->,thick] (project) -- (regionb);
        \draw[->,thick] (regiona) -- (location1);
        \draw[->,thick] (regiona) -- (location2);
        \draw[->,thick] (regionb) -- (location3);
        \draw[->,thick] (regionb) -- (location4);
    \end{tikzpicture}
    \caption{Authorization scope hierarchy.}
    \label{fig:authorization-scope-hierarchy}
\end{figure}
